...
const TokenTable = ({ tokens, onTokenSelected }: TokenTableProps) => {
  ...
  const handleTokenSelection = (token: TokenListResponse) => {
    if (address) {
      selectToken(token);
    } else {
  ...
  return <Table columns={columns} dataSource={tokens} rowKey="id" pagination={{ pageSize: 10 }} onRow={(record) => ({
    onClick: () => handleTokenSelection(record),
  })} />;
};
